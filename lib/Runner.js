...
export default class Runner {
  ...
  [runSyncSimpleTask](task, opts, params) {
    ...
    try {
      ...
      params = inject({params, logger: this.loggers, log: this.loggers}, fn);
  ...
  [runAsyncSimpleTask](task, opts, params) {
    ...
    try {
      ...
      state = (err ? Result.FAILED : ResultState.OK);
    } catch (e) {
      err = e;
      state = ResultState.FAILED;
    } finally {
  ...
  runAsyncFunction(fn, params) {
    ...
    try {
      err = deasync(function(done) {
        ...
        params = inject({done: jdone, params, logger: this.loggers, log: this.loggers}, fn);
        fn(...params);
      })();
    } catch (e) {
      err = e;
  ...
  [suite](...args) {
    ...
    wrapper = (...args) => {
      ...
      //(2) run task
      if (!this.only || (this.only && task.only)) {
        ...
        if (suite.only) {
          if (suite.fullyOnly) {
            ...
            for (let task of suite.tasks) {
              if (task.__task__.only) task(oo, ...params);
            }
          }
        }
      }
