...
export default class Runner {
   ...
   * @return function
   *
   * @overload  From existing simple task.
   * @param opts:object
   */
  [simple](...args) {
  ...
  [runSyncSimpleTask](task, opts, params) {
    ...
    try {
      ...
      params = inject({params, console: this.console}, fn);
  ...
  runAsyncFunction(fn, params) {
    ...
    try {
      sync((done) => {
        ...
        params = inject({done: jdone, params, console: this.console}, fn);
        fn(...params);
      });
    } catch (e) {
      err = e;
  ...
  runWorkflow(workflow, opts, params) {
    ...
    if (opts.ignore) {
      ...
      try {
        let fn = workflow.fn;
        params = inject({params: params, console: this.console}, fn);
